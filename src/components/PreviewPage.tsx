...
export function PreviewPage({ code, onClose }: PreviewPageProps) {
  ...
  // Custom Deploy configurations (Vercel & Netlify)
  ...
  const [customNetlifyToken, setCustomNetlifyToken] = useState(() => {
    try {
      return localStorage.getItem('davecore_netlify_token') || localStorage.getItem('fluxel_netlify_token') || '';
    } catch (e) {
      return '';
    }
  });
  const [customVercelToken, setCustomVercelToken] = useState(() => {
    try {
      return localStorage.getItem('davecore_vercel_token') || '';
    } catch (e) {
      return '';
    }
  });
  ...
  const handlePublish = async () => {
    ...
    const activeToken = activeProvider === 'netlify' ? customNetlifyToken.trim() : customVercelToken.trim();

    try {
      try {
        if (activeProvider === 'netlify') {
          localStorage.setItem('davecore_netlify_token', activeToken);
        } else {
          localStorage.setItem('davecore_vercel_token', activeToken);
        }
      } catch (e) {
        console.error(e);
      }
      ...
      const res = await fetch('/api/publish', {
        ...
        body: JSON.stringify({ 
          html: code,
          customToken: activeToken || undefined,
  ...
  const handleCopyUrl = () => {
    if (publishedUrl) {
      const fullUrl = publishedUrl.startsWith('/') ? window.location.origin + publishedUrl : publishedUrl;
  ...
  const handleOpenNewTab = () => {
    if (tempUrl) {
      const fullUrl = tempUrl.startsWith('/') ? window.location.origin + tempUrl : tempUrl;
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-[#f8f9fa] fixed inset-0 z-50">
      ...
      <header className="flex items-center justify-between px-2 md:px-4 h-14 bg-white border-b border-black/5 shrink-0">
        <div className="flex items-center gap-1 md:gap-3">
          ...
          <div className="flex items-center bg-gray-100 rounded-md px-2 md:px-3 py-1.5 text-xs md:text-sm text-gray-600 border border-gray-200 truncate max-w-[180px] md:max-w-none font-mono">
            ...
            {typeof window !== 'undefined' ? window.location.host : 'davecore-site.live'}/pub/{tempId || 'live'}
          </div>
        </div>
      ...
      <div className="flex-1 overflow-hidden bg-[#FAF9F6] flex items-center justify-center p-0 md:p-8 relative">
        ...
        {/* Custom Deploy Token Publish Modal */}
        <AnimatePresence>
          {showPublishModal && (
            <div className="fixed inset-0 bg-black/45 backdrop-blur-[4px] flex items-center justify-center z-[150] p-4">
              ...
              >
                ...
                <div className="absolute top-0 right-0 w-36 h-36 bg-blue-50/20 rounded-full blur-3xl pointer-events-none" />
                <div className="absolute bottom-0 left-0 w-36 h-36 bg-emerald-50/20 rounded-full blur-3xl pointer-events-none" />

                <div className="flex items-center gap-3 mb-5 relative z-10">
                  <span className="flex items-center justify-center w-10 h-10 rounded-xl bg-blue-50 text-blue-600 shadow-sm border border-blue-100/30">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-gray-400 mt-1 font-sans">Hosting Instan, Aman & Berkinerja Tinggi</p>
                  </div>
                </div>

                {/* Provider Tab Selector */}
                <div className="flex bg-gray-100/80 p-1 rounded-xl mb-5 relative z-10">
                  <button
                    type="button"
                ...
                {/* Account customization & token input */}
                <div className="bg-[#FAF9F6] border border-gray-150/60 rounded-2xl p-4.5 mb-6 relative z-10 text-left shadow-sm">
                  <h4 className="text-xs font-bold text-gray-850 mb-1 flex items-center gap-1.5 font-sans">
                    <Key className="w-3.5 h-3.5 text-blue-650" /> Token API {activeProvider === 'vercel' ? 'Vercel' : 'Netlify'} (Wajib)
                  </h4>
                  <p className="text-[11px] text-gray-400 mb-3.5 leading-relaxed font-sans">
                    Masukkan token API {activeProvider === 'vercel' ? 'Vercel' : 'Netlify'} Anda agar website dideploy langsung ke akun pribadi Anda.
                  </p>
                  
                  {activeProvider === 'vercel' ? (
                    <input
                      type="password"
                      placeholder="Contoh: v1.aB1cD2eF3gH4..."
                      value={customVercelToken}
                      onChange={(e) => setCustomVercelToken(e.target.value)}
                      className="w-full px-3 py-2 bg-white border border-gray-250 rounded-xl text-xs font-mono text-gray-800 placeholder-gray-300 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/50 transition-all shadow-inner"
                    />
                  ) : (
                    <input
                      type="password"
                  ...
                  <div className="mt-2.5 flex items-center justify-between text-[10px]">
                    <span className="text-amber-600 font-semibold flex items-center gap-1">
                      <AlertTriangle className="w-3 h-3 shrink-0" /> Token wajib diisi.
                    </span>
                    {activeProvider === 'vercel' ? (
                      <a 
                        ...
                        className="text-blue-650 hover:text-blue-700 hover:underline font-semibold font-sans flex items-center gap-0.5"
                      >
                        Dapatkan Token ↗
                      </a>
                    ) : (
                ...
                <div className="flex gap-3 relative z-10">
                  ...
                  <button
                    ...
                    disabled={activeProvider === 'vercel' ? !customVercelToken.trim() : !customNetlifyToken.trim()}
                    className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-semibold transition-all flex items-center justify-center gap-1.5 shadow-sm font-sans disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-blue-600"
                  >
                    Mulai Deploy
                  </button>
                </div>
              ...
              >
                ...
                <div className="flex items-center justify-between p-3 bg-gray-50 border border-gray-155 rounded-xl mb-5 shadow-inner">
                  <span className="text-xs font-mono text-gray-650 truncate max-w-[70%] select-all px-1">
                    {publishedUrl.startsWith('/') ? window.location.origin + publishedUrl : publishedUrl}
                  </span>
                  <button
                ...
                {isNetlify ? (
                  ...
                  <div className="mb-6 p-3 bg-amber-50/80 border border-amber-100/50 rounded-xl flex items-start gap-2.5 shadow-sm">
                    ...
                    <p className="text-[11px] text-amber-850 leading-relaxed font-sans">
                      <strong className="font-semibold text-amber-900 font-sans">Mode Preview Publik:</strong> Website dideploy menggunakan server lokal DAVECORE sementara. Tambahkan <code className="bg-amber-100/80 px-1 py-0.5 rounded font-mono font-semibold text-[10px]">NETLIFY_TOKEN</code> di Settings (Secrets) untuk mendapatkan domain permanen Netlify Anda secara otomatis!
                    </p>
                  </div>
                )}
                ...
                <div className="flex gap-3">
                  ...
                  <a
                    href={publishedUrl.startsWith('/') ? window.location.origin + publishedUrl : publishedUrl}
                    target="_blank"
                    rel="noopener noreferrer"
