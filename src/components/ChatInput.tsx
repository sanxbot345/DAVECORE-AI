...
export function ChatInput({ onSend, disabled, placeholder }: ChatInputProps) {
  ...
  return (
    <div className="w-full max-w-5xl bg-white border border-claude-border rounded-[24px] shadow-lg p-4 md:p-5 flex flex-col gap-2.5 relative z-30">
      
      {/* Attached File Preview Badge */}
      {attachedFile && (
        <div className="relative max-w-sm flex items-center gap-3 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-xl p-3 group select-none self-start hover:border-red-200 transition-all shadow-sm">
          <div className="p-2.5 bg-white border border-gray-200 rounded-lg shadow-sm text-claude-accent">
            <File className="w-6 h-6" />
          </div>
          <div className="flex flex-col pr-6 min-w-0">
            <span className="text-xs font-semibold text-gray-800 truncate max-w-[220px]" title={attachedFile.name}>
              {attachedFile.name}
            </span>
            <span className="text-[10px] text-gray-500 font-mono">
              {attachedFile.content ? `${(attachedFile.content.length / 1024).toFixed(1)} KB` : 'File Terlampir'}
            </span>
          </div>
          <button
            onClick={() => setAttachedFile(null)}
            className="absolute top-2 right-2 w-5 h-5 rounded-full bg-white border border-gray-200 shadow-sm hover:bg-red-50 text-gray-500 hover:text-red-600 transition-all flex items-center justify-center scale-90 hover:scale-110"
            title="Hapus file"
          >
            <X size={12} />
          </button>
